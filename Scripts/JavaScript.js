...
function CaptainsPick(legion, hellbourne, phases) {
    ...
    this.isEnd = function () {
        return !(instance.phaseIndex >= 0 && instance.phaseIndex < instance.phases.length) ||
            (instance.currentTime <= 0 && instance.currentPhase().team.bonusRemainingTime <= 0);
    };
    ...
    this.update = function () {
        if ((instance.currentTime <= 0 && instance.currentPhase().team.bonusRemainingTime <= 0))
            instance.writeAlert(instance.currentPhase().team.writeName + " ran out of time!");
    ...
    this.tick = function () {
        if (!instance.isEnd()) {
            if (instance.currentTime > 0)
                ...
                if (instance.currentPhase().team.bonusRemainingTime > 0)
                    instance.currentPhase().team.bonusRemainingTime--;
                else {
                    instance.stop();
                }
            }
    ...
    this.heroClicked = function (hero) {
        if (!instance.isEnd() && !$(hero).hasClass("banned") && !$(hero).hasClass("picked") && !$(hero).hasClass("not-selectable")) {
            ...
            instance.nextPhase();
            if (instance.isEnd())
                instance.writeAlert("Picking phase is done!");
        }
    };
